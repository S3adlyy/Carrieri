...
public class HelloApplication extends Application {
    ...
    private static final String ALERT_CSS_PATH = "/com/example/guser/gcommu/Alert.css";
    private static final String SHELL_FXML_PATH = "/com/example/guser/gcommu/mission-shell.fxml";
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        new Thread(() -> {
            try {
                ...
                updateProgress(0.2, "🎓 Initialisation de l'application...");
                Thread.sleep(300);
                ...
                // Étape 3: Préparation de l'interface
                updateProgress(0.6, "💬 Préparation de la messagerie...");
                Thread.sleep(300);

                // Étape 4: Chargement du shell principal (MissionShell)
                updateProgress(0.8, "⚙️ Chargement de l'interface...");

                // Charger le FXML
                FXMLLoader loader = new FXMLLoader(getClass().getResource(SHELL_FXML_PATH));

                // IMPORTANT: Si le chemin ne fonctionne pas, essayez des chemins alternatifs
                ...
                updateProgress(1.0, "🚀 Démarrage de Carrieri...");
                Thread.sleep(300);
                ...
                Platform.runLater(() -> {
                    ...
                    if (getClass().getResource(ALERT_CSS_PATH) != null) {
                        scene.getStylesheets().add(getClass().getResource(ALERT_CSS_PATH).toExternalForm());
                        System.out.println("✅ CSS Alert chargé: " + ALERT_CSS_PATH);
                    } else {
                        System.err.println("⚠️ CSS Alert non trouvé: " + ALERT_CSS_PATH);
                    }

                    // Ajouter également le ThemeUnified.css s'il existe
                    String themeCssPath = "/ThemeUnified.css";
                    if (getClass().getResource(themeCssPath) != null) {
                        scene.getStylesheets().add(getClass().getResource(themeCssPath).toExternalForm());
                        System.out.println("✅ Theme CSS chargé: " + themeCssPath);
                    }

                    primaryStage.setScene(scene);
                    primaryStage.setTitle("Carrieri - Communication Platform");
    ...
    private void createSplashScreen() {
        ...
        // Sous-titre
        Label subtitleLabel = new Label("Communication Platform");
    ...
    private StackPane createLogoWithFallback() {
        ...
        if (logoImage != null) {
            ...
            Label emojiLabel = new Label("💬");  // Changed to chat icon for communication platform
            emojiLabel.setStyle(
                "-fx-font-size: 48px;" +
                    "-fx-text-fill: #231942;"
            );
