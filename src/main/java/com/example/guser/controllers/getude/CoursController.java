...
import services.getude.LeconService;
import services.getude.ModuleService;
import services.getude.QuizAutoGenerator;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        tableCours.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
            if (newSelection != null) {
                ...
                AppNavController.getInstance().setEtudeCurrentCours(newSelection.getId(), newSelection.getTitre());
...
// Lier le champ prix à la case à cocher
        ...
        if (confirmed) {
            try {
                ...
                if (coursSelectionne != null && coursSelectionne.getId() == cours.getId()) {
                    AppNavController.getInstance().resetEtudeCours();
                }
    ...
    public void genererTestFinalAutomatique() {
        ...
        List<entities.getude.Module> modules = moduleService.getModulesByCours(coursSelectionne.getId());
        ...
        boolean hasLecons = modules.stream().anyMatch(m -> {
            List<entities.getude.Lecon> lecons = leconService.getLeconsByModule(m.getId());
