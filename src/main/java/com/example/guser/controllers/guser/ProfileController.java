...
public class ProfileController {
    ...
    @FXML private Button aboutAiBtn;
    @FXML private Label aboutAiHintLabel;
...
// placeholders until backend exists:
    ...
    @FXML
    private void onAboutImproveWithAI() {
        if (!owner) return;
        hideError();

        if (bioArea == null) return;

        String current = bioArea.getText();
        if (current == null || current.trim().isEmpty()) {
            showError("Write something in Bio first, then click Improve with AI.");
            return;
        }

        if (aboutAiBtn != null) aboutAiBtn.setDisable(true);
        if (aboutAiHintLabel != null) {
            aboutAiHintLabel.setText("Improving…");
            aboutAiHintLabel.setVisible(true);
            aboutAiHintLabel.setManaged(true);
        }

        javafx.concurrent.Task<String> task = new javafx.concurrent.Task<>() {
            @Override
            protected String call() throws Exception {
                return new services.guser.AboutAIApiService().improveAbout(current);
            }
        };
        ...
        task.setOnSucceeded(e -> {
            ...
            if (improved != null && !improved.isBlank()) {
                bioArea.setText(improved.trim());
                if (aboutAiHintLabel != null) aboutAiHintLabel.setText("Done. Review then click Save.");
            } else {
                if (aboutAiHintLabel != null) aboutAiHintLabel.setText("No suggestion returned.");
            }
            if (aboutAiBtn != null) aboutAiBtn.setDisable(false);
        });

        task.setOnFailed(e -> {
            Throwable ex = task.getException();
            if (aboutAiHintLabel != null) {
                aboutAiHintLabel.setText("AI failed: " + (ex == null ? "unknown error" : ex.getMessage()));
            }
            if (aboutAiBtn != null) aboutAiBtn.setDisable(false);
        });

        Thread t = new Thread(task, "about-ai-task");
        t.setDaemon(true);
        t.start();
    }
    ...
    //============NAVBAR HELPERS FOR INSTANCIATION====================
    private static void go(String fxml, String title) {
        AppNavController nav = AppNavController.getInstance();
        if (nav == null) return; // safety if someone loads profile standalone
        nav.show(fxml, title);
    }
    private static boolean ensureLoggedInOrRedirect() {
        if (SessionContext.isLoggedIn()) return true;
        AppNavController nav = AppNavController.getInstance();
        if (nav != null) nav.show("/com/example/guser/guser/login.fxml", "Carrieri • Sign in");
        return false;
    }
