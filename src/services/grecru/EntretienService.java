...
public class EntretienService {
    ...
    private static final String COMPANY_PREFIX = "Carrieri-";
     ...
     */
    private String generateJitsiRoomName(int candidatId, int missionId, int entretienId) {
        // Format: Carrieri-CANDIDAT123-MISSION45-ENTRETIEN789-TIMESTAMP
        String timestamp = String.valueOf(System.currentTimeMillis()).substring(7); // Last 4 digits
        return COMPANY_PREFIX + "CANDIDAT" + candidatId +
                "-MISSION" + missionId +
    ...
    /**
     * Generate full Jitsi Meet URL
     */
    private String generateJitsiMeetUrl(int candidatId, int missionId, int entretienId) {
        String roomName = generateJitsiRoomName(candidatId, missionId, entretienId);
        return JITSI_BASE_URL + roomName;
    }

    /**
     * Create a new entretien and send email with Jitsi Meet link
     */
    public void createEntretienWithJitsi(Entretien entretien, String candidatEmail, String candidatName,
                                         int candidatId, int missionId) throws Exception {
        ...
        try {
            ...
            String jitsiUrl = generateJitsiMeetUrl(candidatId, missionId, entretien.getId());

            // Send email with Jitsi link
            sendEntretienEmailWithJitsi(entretien, candidatEmail, candidatName, jitsiUrl);
    ...
    /**
     * Send email with Jitsi Meet link
     */
    private void sendEntretienEmailWithJitsi(Entretien entretien, String candidatEmail,
                                             String candidatName, String jitsiUrl) {
        final String username = FROM_EMAIL;
        final String password = APP_PASSWORD;

        System.out.println("📧 Attempting to send email with Jitsi link...");
        System.out.println("   From: " + username);
        System.out.println("   To: " + candidatEmail);
        System.out.println("   Jitsi URL: " + jitsiUrl);
        ...
        // Try SSL first
        try {
            sendEmailWithPropsAndJitsi(props1, username, password, candidatEmail, candidatName, entretien, jitsiUrl);
            ...
            try {
                sendEmailWithPropsAndJitsi(props2, username, password, candidatEmail, candidatName, entretien, jitsiUrl);
        ...
        System.out.println("\n📧 ============ EMAIL PREVIEW WITH JITSI ============");
        System.out.println("To: " + candidatEmail);
        System.out.println(createJitsiEmailBody(entretien, candidatName, jitsiUrl));
        System.out.println("========================================\n");
    }

    private void sendEmailWithPropsAndJitsi(Properties props, String username, String password,
                                            String toEmail, String candidatName,
                                            Entretien entretien, String jitsiUrl)
        ...
        message.setSubject("🎥 Invitation - Entretien Visio avec Jitsi Meet | Carrieri");
        message.setSentDate(new java.util.Date());

        String emailBody = createJitsiEmailBody(entretien, candidatName, jitsiUrl);
        message.setContent(emailBody, "text/html; charset=utf-8");

        Transport.send(message);
    }

    private String createJitsiEmailBody(Entretien entretien, String candidatName, String jitsiUrl) {
        ...
        return "<!DOCTYPE html>" +
                ...
                ".container { max-width: 600px; margin: 20px auto; background: linear-gradient(135deg, #f8f9fa 0%, #ffffff 100%); border-radius: 24px; overflow: hidden; box-shadow: 0 20px 40px rgba(35,25,66,0.2); }" +
                ...
                /* Jitsi Meeting Card */
                ".jitsi-card { background: linear-gradient(135deg, #f0e7ff, #e5d9f2); border-radius: 20px; padding: 30px; margin: 25px 0; text-align: center; border: 2px solid #5E548E; }" +
                ".jitsi-card h3 { color: #231942; font-size: 22px; margin-top: 0; }" +
                ".jitsi-button { display: inline-block; background: #10b981; color: white; padding: 18px 40px; text-decoration: none; border-radius: 60px; font-weight: 800; font-size: 18px; margin: 20px 0; box-shadow: 0 10px 20px rgba(16,185,129,0.3); border: none; }" +
                ".jitsi-button:hover { background: #059669; }" +
                ".jitsi-info { background: white; border-radius: 16px; padding: 20px; margin: 20px 0; }" +
                ".jitsi-info p { margin: 5px 0; color: #231942; }" +
                ".jitsi-url { background: #231942; color: white; padding: 12px; border-radius: 12px; font-family: monospace; font-size: 14px; word-break: break-all; }" +
                ...
                /* Instructions for Jitsi */
                ".instructions { background: #f8f9fa; border-radius: 16px; padding: 20px; margin: 25px 0; }" +
                ".instructions h4 { color: #231942; margin-top: 0; }" +
                ".instructions ul { padding-left: 20px; color: #555; }" +
                ...
                "<div class='header'>" +
                "<h1>🎥 Carrieri - Entretien Visio</h1>" +
                ...
                "Nous avons le plaisir de vous inviter à un entretien vidéo via <strong>Jitsi Meet</strong>." +
                "</p>" +

                "<!-- Jitsi Meeting Card - MAIN FEATURE -->" +
                "<div class='jitsi-card'>" +
                "<h3>🎥 Votre salle de visio</h3>" +

                "<div class='jitsi-info'>" +
                "<p style='font-size: 16px; color: #231942;'>Cliquez sur le bouton ci-dessous pour rejoindre :</p>" +
                "<a href='" + jitsiUrl + "' class='jitsi-button' target='_blank'>🚀 REJOINDRE LA VISIO</a>" +
                "<p style='color: #5E548E; margin-top: 15px; font-size: 14px;'>La réunion s'ouvrira dans votre navigateur - pas d'installation requise</p>" +
                "</div>" +

                "<div class='jitsi-url'>" +
                "🔗 Lien direct: " + jitsiUrl +
                "</div>" +
                "</div>" +
                ...
                "<h4>📌 Comment utiliser Jitsi Meet :</h4>" +
                "<ul>" +
                "<li>✅ Aucune installation nécessaire - ça marche dans votre navigateur</li>" +
                "<li>🎥 Autorisez l'accès à votre caméra et microphone quand demandé</li>" +
                "<li>💬 Vous pouvez partager votre écran si besoin</li>" +
                "<li>📱 Compatible avec Chrome, Firefox, Safari et mobile</li>" +
                ...
                "Pour toute question technique sur Jitsi Meet :<br>" +
                ...
                "<small>Entretien vidéo sécurisé via Jitsi Meet</small>" +
                "</div>" +
    ...
    /**
     * Send regular email (without Jitsi) - keeping for backward compatibility
     */
